...
const Bar = ({ label, val, total, color }) => {
  ...
  return (
    <div className="an-bar-wrap">
      <div className="an-bar-label"><span>{label}</span><span style={{color}}>{val} ({p}%)</span></div>
      <div className="an-bar-track"><div className="an-bar-fill" style={{ width:`${p}%`, background:color }} /></div>
    </div>
  )
}
...
export default function Analytics({ user }) {
  ...
  async function run() {
    ...
    let totalSpins=0,red=0,black=0,odd=0,even=0,high=0,low=0,zero=0
    let ZV=0,V=0,O=0,T=0,d1=0,d2=0,d3=0,r1=0,r2=0,r3=0
    let s1=0,s2=0,s3=0,s4=0,z1=0,z2=0
    const numCount = {}

    rows.forEach(r => {
      const s = r.stats || {}
      totalSpins+=r.total_spins||0; red+=s.red||0; black+=s.black||0
      odd+=s.odd||0; even+=s.even||0; high+=s.high||0; low+=s.low||0; zero+=s.zero||0
      ...
      ;(r.numbers||[]).forEach(n => { numCount[n] = (numCount[n]||0)+1 })
    })
      ...
      {data && <>
        <div className="list-box">
          ...
          <div className="stat-grid">
            {[['Sessions',data.sessions],['Total Spins',data.totalSpins],['Avg Spins',Math.round(data.totalSpins/data.sessions)],['Zero',data.zero]].map(([l,v]) => (
              <div key={l} className="stat-box"><span className="stat-lbl">{l}</span><span className="stat-val">{v}</span></div>
            ))}
          </div>
        </div>
        ...
        <div className="list-box">
          <div className="box-header">🔥 Hot Numbers</div>
          <div className="hot-cold-grid">
            {data.hot.map(([n,c]) => { const bg=Number(n)===0?'#00cc66':REDS.includes(Number(n))?'#c0202e':'#333'; return <div key={n}><div className="hc-ball" style={{background:bg,color:'#fff'}}>{n}</div><div style={{textAlign:'center',fontSize:'0.6rem',color:'#d4af37',marginTop:'2px'}}>{c}</div></div> })}
          </div>
        </div>
        <div className="list-box">
          <div className="box-header">❄️ Cold Numbers</div>
          <div className="hot-cold-grid">
            {data.cold.map(([n,c]) => { const bg=Number(n)===0?'#00cc66':REDS.includes(Number(n))?'#c0202e':'#333'; return <div key={n}><div className="hc-ball" style={{background:bg,color:'#fff'}}>{n}</div><div style={{textAlign:'center',fontSize:'0.6rem',color:'#d4af37',marginTop:'2px'}}>{c}</div></div> })}
          </div>
        </div>
      </>}

      {!loading && !data && (
        <div style={{ color:'#666', fontSize:'0.8rem', textAlign:'center', padding:'20px' }}>Set filters and click Run Analytics</div>
      )}
    </div>
  )
}
