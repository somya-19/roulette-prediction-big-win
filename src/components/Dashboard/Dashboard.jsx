...
export default function Dashboard({ history }) {
  ...
  const cards = [
    ...
    { lbl:'Row 1/2/3',            val: `${v.filter(x=>x%3===1).length} / ${v.filter(x=>x%3===2).length} / ${v.filter(x=>x%3===0).length}`, color:'#d4af37', small:true },
    { lbl:'Dozen 1/2/3',          val: `${v.filter(x=>x<=12).length} / ${v.filter(x=>x>=13&&x<=24).length} / ${v.filter(x=>x>=25).length}`, color:'#d4af37', small:true },
    ...
    <div className="dashboard">
      {cards.map((c, i) => (
        <div className="card" key={i}>
          ...
          {c.val !== null
            ? <div className="val" style={{ color:c.color, fontSize:c.small?'0.82rem':'1rem' }}>{c.val}</div>
